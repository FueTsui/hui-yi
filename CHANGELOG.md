...
## 1.2.2 — Fix bundled hook template installation

- Fixed `scripts/install_hook.py` so bundled hook templates now come from `skills/hui-yi/hooks/hui-yi-signal-hook/` instead of incorrectly reading from the already-installed workspace hook directory.
- Added the real `HOOK.md` and `handler.ts` hook templates into the skill package so installation no longer reuses stale placeholder files.
- Preserved `--enable` behavior, so hook file installation and hook config enablement still work in one step.
- This fixes cases where `install_hook.py` appeared to succeed but left users with placeholder hook contents.

## 1.2.1 — Hook enablement during installation

- Extended `scripts/install_hook.py` with `--enable` so Hui-Yi can install hook files and enable the workspace hook configuration in one step.
- Added `--config-path` support to target a specific `openclaw.json` when needed.
- Kept the installer idempotent: if hook files already exist or the hook is already enabled, it reports status instead of rewriting unnecessarily.
- Updated documentation to show the new one-step hook install + enable flow.

## 1.2.0 — Repetition-first integration and release cleanup

- Clarified Hui-Yi as a repetition-first, Ebbinghaus-paced cold-memory reinforcement layer instead of a date-only review tool.
- Added and documented real-session signal tooling, including `signal_detect.py`, `signal_pipeline.py`, `openclaw_signal_hook.py`, and `openclaw_runtime_probe.py`.
- Added `install_hook.py` to support explicit Hui-Yi hook installation into `hooks/hui-yi-signal-hook/`.
- Expanded `review.py` workflows around resurfacing, feedback writeback, and session-aware reinforcement.
- Updated `README.md`, `SKILL.md`, and integration references to reflect OpenClaw memory boundaries, bridge behavior, and hook-based integration design.
- Added `version: 1.2.0` to `manifest.yaml` for release metadata consistency.
