...
- `bridge.py` 负责策略筛选、去重、限流、投递适配
- `delivery.mode = message` 目前仍是集成位，不应当把它当成现成的稳定生产方案
...
行为说明：
- 默认会把 skill 内置模板安装到 `workspace/hooks/hui-yi-signal-hook/`
- 模板来源是 `skills/hui-yi/hooks/hui-yi-signal-hook/`，不是 workspace 中旧的已安装文件
