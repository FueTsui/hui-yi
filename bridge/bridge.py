...
def resolve_path(value: str) -> Path:
    ...
    candidates = [(WORKSPACE_ROOT / path).resolve()]
    if len(path.parts) >= 2 and path.parts[0] == "skills" and path.parts[1] == SKILL_ROOT.name:
        candidates.append((SKILL_ROOT / Path(*path.parts[2:])).resolve())
    else:
        candidates.append((SKILL_ROOT / path).resolve())
...
def policy_allows_candidate(candidate: dict, state: dict, config: dict, now_dt: datetime) -> tuple[bool, str]:
    ...
    dedupe_hours = float(policy.get("dedupeHours", 72) or 72)
    previous_delivery = last_delivered.get(key)
    if within_hours(now_dt, previous_delivery, dedupe_hours):
        return False, "duplicate"

    if key in last_delivered and not within_hours(now_dt, previous_delivery, dedupe_hours):
        last_delivered.pop(key, None)
...
def main() -> int:
    ...
    state["lastDelivered"] = {
        key: ts
        for key, ts in (state.get("lastDelivered", {}) if isinstance(state.get("lastDelivered"), dict) else {}).items()
        if within_hours(now_dt, ts, dedupe_hours)
    }
